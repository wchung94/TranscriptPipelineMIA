...
def set_arguments():
    ...
    default= '/local/data/course/project/groups/nijveen/FINAL/Final_WithBraker/augustus.gff',\
     metavar=".gff")
...
#### change filename to hisat_out (better readability
    ...
    for templabel in temp_infile_list:
        cmd = "hisat2 -p 8 --dta-cufflinks -x {} -U {} -S {}".format(index_file, "../{}/".format(fastqdir) + templabel + "_1.fastq", templabel + ".sam")
...
##    # CUFFDIFF
##    if os.path.exists("cuffdiff/gene_exp.diff") == False:
##        run_cuffdiff(args.annotation, label_dict, "../hisat2_sam/")
##
##    os.chdir('..')
##
    # CUFFDIFF-PARSE
    diffparser("cuffdiff/gene_exp.diff")
    print(datetime.now(), "Parsing of cuffdiff output completed. Outputs in DiffParserOut.txt")

    #output-parse prep
    print(datetime.now(), "\t>Start Output Analysis<")
    MIA_analysis_dir = str(datetime.now())[:19].replace(' ','_')
    try:
    ...
    run_MIA_analysis('../cuffdiff/gene_exp.diff','../CRO_names.txt') #COULD turn this into an argument variable
